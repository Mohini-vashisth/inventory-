...
Admin enters company name/email/phone in the Orders dashboard (or sends a quote from a logged Query, see above) → sends a unique link via email or a copied link → customer fills the form (pre-filled if it came from a Query) → order is created with status `pending`, linked back via `source_query` if applicable → admin reviews. The quote link is **single-use**: the token regenerates after submission, making the old URL a 404.
