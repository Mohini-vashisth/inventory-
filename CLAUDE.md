...
Before any formal quote exists, an inbound inquiry (usually a phone call) is logged as a `Query` via the **Query Dashboard** (`materials/views.py::query_dashboard`). It deliberately does **not** link to `/orders/` — Queries and Orders are separate staff workflows, not a browsing path into each other. Manual logging only asks for **source + phone number** — nothing else is known yet, so nothing else is asked. Submitting it immediately fires an automated WhatsApp intake sequence (below) that gathers the rest directly from the customer; there is still no in-dashboard edit view, so an admin override (`/admin/materials/query/`) remains the fallback for a field that needs correcting by hand. Everywhere `company_name` might be blank, code and templates fall back to `contact_phone` (or `f"Query #{pk}"` as a last resort): `Query.__str__`, `query_send_quote`'s customer-name fallback, `QueryAdmin.display_name`, and the dashboard table's Company/Contact column.
