...
- **`query_send_quote`** (staff-only, POST, unchanged by the WhatsApp work below) — creates or reuses a `Customer` from the query's company name (falling back to phone/`Query #pk` if no name yet), copies over email/phone if given, sets `query.customer` and `query.status = 'quote_sent'`. If the customer has an email on file it sends the real quote email (`_dispatch_quote_email`); if not, it shows a warning telling staff to use the existing "Copy Link" action instead.
...
- **Question sequence**: `WHATSAPP_QUERY_FIELDS = ['company_name', 'contact_email', 'grade', 'size']` — `_next_expected_query_field` returns whichever is still blank, in that order. There's no separate "stage" field; the next question is always inferred from the data itself, so it can't drift out of sync and tolerates a field getting filled some other way (e.g. an admin edit) mid-sequence.
- **Size parsing is lenient** (`_parse_whatsapp_size`): `"1.2"` or `"1.2mm"` both become `Decimal('1.2')`; anything with no usable digits doesn't save and just re-sends the size question instead of storing garbage.
...
- **This is the app's second public, unauthenticated endpoint** (after the quote form) — Django's own auth plays no role, since Meta's servers never carry a session or CSRF cookie:
  - **GET** (`_whatsapp_verify`) — the one-time subscription handshake. Confirms `hub.mode == "subscribe"` and `hub.verify_token` matches `WHATSAPP_VERIFY_TOKEN` (constant-time compare), echoes back `hub.challenge`.
  - **POST** (`_whatsapp_receive`) — verifies `X-Hub-Signature-256` as an HMAC-SHA256 of the *raw* body, keyed with `WHATSAPP_APP_SECRET`, **before** the body is parsed at all. `@csrf_exempt` is safe here specifically because this signature check is the real authentication, not Django's CSRF middleware.
  - `value.statuses` (delivery receipts) are ignored, not treated as messages; only text messages are handled, non-text types are skipped for now.
